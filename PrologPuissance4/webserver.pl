...
indexAction(Request) :-
    http_reply_from_files('C:/Users/Gloria/Documents/4IF/ALIA/PrologPuissance4Final/PrologPuissance4/web/pages', [], Request).
indexAction(Request) :-
    http_404([], Request).
...
fichierAction(Request) :-
    http_reply_from_files('C:/Users/Gloria/Documents/4IF/ALIA/PrologPuissance4Final/PrologPuissance4/web/assets', [], Request).
fichierAction(Request) :-
    http_404([], Request).
...
%1 == Joueur
%2 == IA Aléatoire
%3 == MiniMax profondeur 3 : Position
%4 == MiniMax profondeur 3 : Position + Puissance 3
%5 == MiniMax profondeur 4 : Position
%6 == MiniMax profondeur 4 : Position + Puissance 3
%7 ==
%
%
%
%9 == MiniMax profondeur 3 : Gloria
...
obtenirCoup(_,2,Coup) :-
    ...
    % iaMinimax(JoueurCourant,Coup,Profondeur,PoidsPosition,PoidsPuissance3,PoidsDensite,PoidsAdjacence, PoidsHeuristiqueGloria, PoidsAdjacenceLigne, PoidsAdjacenceCarre, PoidsNePasPerdre)
obtenirCoup(CouleurJCourant,3,Coup) :-
    iaMinimax(CouleurJCourant,Coup,1,1,0,0,0,0,0,0,0).
obtenirCoup(CouleurJCourant,4,Coup) :-
    iaMinimax(CouleurJCourant,Coup,1,1,1,0,0,0,0,0,0).
obtenirCoup(CouleurJCourant,5,Coup) :-
    iaMinimax(CouleurJCourant,Coup,2,1,0,0,0,0,0,0,0).
obtenirCoup(CouleurJCourant,6,Coup) :-
    iaMinimax(CouleurJCourant,Coup,2,1,1,0,0,0,0,0,0).

obtenirCoup(CouleurJCourant,7,Coup) :-
    iaMinimax(CouleurJCourant,Coup,3,1,0,0,0,0,0,0,0).
obtenirCoup(CouleurJCourant,8,Coup) :-
    iaMinimax(CouleurJCourant,Coup,3,1,1,0,0,0,0,0,0).
obtenirCoup(CouleurJCourant,9,Coup) :-
    iaMinimax(CouleurJCourant,Coup,4,1,0,0,0,0,0,0,0).
obtenirCoup(CouleurJCourant,10,Coup) :-
    iaMinimax(CouleurJCourant,Coup,4,1,1,0,0,0,0,0,0).

%Heuristique Combinaisons
obtenirCoup(CouleurJCourant, 11, Coup) :-
    iaMinimax(CouleurJCourant,Coup,1,0,0,0,0,1,0,0,0).
obtenirCoup(CouleurJCourant, 12, Coup) :-
    iaMinimax(CouleurJCourant,Coup,1,0,1,0,0,1,0,0,0).
obtenirCoup(CouleurJCourant, 13, Coup) :-
    iaMinimax(CouleurJCourant,Coup,2,0,0,0,0,1,0,0,0).
obtenirCoup(CouleurJCourant, 14, Coup) :-
    iaMinimax(CouleurJCourant,Coup,2,0,1,0,0,1,0,0,0).

obtenirCoup(CouleurJCourant, 15, Coup) :-
    iaMinimax(CouleurJCourant,Coup,3,0,0,0,0,1,0,0,0).
obtenirCoup(CouleurJCourant, 16, Coup) :-
    iaMinimax(CouleurJCourant,Coup,3,0,1,0,0,1,0,0,0).
obtenirCoup(CouleurJCourant, 17, Coup) :-
    iaMinimax(CouleurJCourant,Coup,4,0,0,0,0,1,0,0,0).
obtenirCoup(CouleurJCourant, 18, Coup) :-
    iaMinimax(CouleurJCourant,Coup,4,0,1,0,0,1,0,0,0).

%Heuristique Adjacence Ligne
obtenirCoup(CouleurJCourant, 19, Coup) :-
    iaMinimax(CouleurJCourant,Coup,1,0,0,0,0,0,1,0,0).
obtenirCoup(CouleurJCourant, 20, Coup) :-
    iaMinimax(CouleurJCourant,Coup,1,0,1,0,0,0,1,0,0).
obtenirCoup(CouleurJCourant, 21, Coup) :-
    iaMinimax(CouleurJCourant,Coup,2,0,0,0,0,0,1,0,0).
obtenirCoup(CouleurJCourant, 22, Coup) :-
    iaMinimax(CouleurJCourant,Coup,2,0,1,0,0,0,1,0,0).

obtenirCoup(CouleurJCourant, 23, Coup) :-
    iaMinimax(CouleurJCourant,Coup,3,0,0,0,0,0,1,0,0).
obtenirCoup(CouleurJCourant, 24, Coup) :-
    iaMinimax(CouleurJCourant,Coup,3,0,1,0,0,0,1,0,0).
obtenirCoup(CouleurJCourant, 25, Coup) :-
    iaMinimax(CouleurJCourant,Coup,4,0,0,0,0,0,1,0,0).
obtenirCoup(CouleurJCourant, 26, Coup) :-
    iaMinimax(CouleurJCourant,Coup,4,0,1,0,0,0,1,0,0).


%Heuristique Adjacence Carré
obtenirCoup(CouleurJCourant, 27, Coup) :-
    iaMinimax(CouleurJCourant,Coup,1,0,0,0,0,0,0,1,0).
obtenirCoup(CouleurJCourant, 28, Coup) :-
    iaMinimax(CouleurJCourant,Coup,1,0,1,0,0,0,0,1,0).
obtenirCoup(CouleurJCourant, 29, Coup) :-
    iaMinimax(CouleurJCourant,Coup,2,0,0,0,0,0,0,1,0).
obtenirCoup(CouleurJCourant, 30, Coup) :-
    iaMinimax(CouleurJCourant,Coup,2,0,1,0,0,0,0,1,0).
obtenirCoup(CouleurJCourant, 31, Coup) :-
    iaMinimax(CouleurJCourant,Coup,3,0,0,0,0,0,0,1,0).
obtenirCoup(CouleurJCourant, 32, Coup) :-
    iaMinimax(CouleurJCourant,Coup,3,0,1,0,0,0,0,1,0).
obtenirCoup(CouleurJCourant, 33, Coup) :-
    iaMinimax(CouleurJCourant,Coup,4,0,0,0,0,0,0,1,0).
obtenirCoup(CouleurJCourant, 34, Coup) :-
    iaMinimax(CouleurJCourant,Coup,4,0,1,0,0,0,0,1,0).

%Heuristique Ne pas perdre
obtenirCoup(CouleurJCourant, 35, Coup) :-
    iaMinimax(CouleurJCourant,Coup,1,0,0,0,0,0,0,0,1).
obtenirCoup(CouleurJCourant, 36, Coup) :-
    iaMinimax(CouleurJCourant,Coup,1,0,1,0,0,0,0,0,1).
obtenirCoup(CouleurJCourant, 37, Coup) :-
    iaMinimax(CouleurJCourant,Coup,2,0,0,0,0,0,0,0,1).
obtenirCoup(CouleurJCourant, 38, Coup) :-
    iaMinimax(CouleurJCourant,Coup,2,0,1,0,0,0,0,0,1).
obtenirCoup(CouleurJCourant, 39, Coup) :-
    iaMinimax(CouleurJCourant,Coup,3,0,0,0,0,0,0,0,1).
obtenirCoup(CouleurJCourant, 40, Coup) :-
    iaMinimax(CouleurJCourant,Coup,3,0,1,0,0,0,0,0,1).
obtenirCoup(CouleurJCourant, 41, Coup) :-
    iaMinimax(CouleurJCourant,Coup,4,0,0,0,0,0,0,0,1).
obtenirCoup(CouleurJCourant, 42, Coup) :-
    iaMinimax(CouleurJCourant,Coup,4,0,1,0,0,0,0,0,1).
